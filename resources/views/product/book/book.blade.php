        ...
        <div class="container">
            ...
            <div class="catalog-grid">
                @forelse ($items as $item)
                    <div class="product-card animate-fade-in" data-category="{{ $item->type ?? 'other' }}">
                        <div class="product-image">
                            <img src="{{ Str::startsWith($item->img, ['http://', 'https://']) ? $item->img : asset('storage/' . $item->img) }}"
                                width="60" class="img-fluid rounded-top" alt="Gambar {{ $item->name }}"
                                onerror="this.onerror=null;this.src='{{ asset('images/default.png') }}';">
                        </div>

                        <div class="product-content">
                            <h3 class="product-title">{{ $item->name }}</h3>
                            <p class="product-description">{{ Str::limit($item->description, 100) }}</p>

                            <div class="product-footer">
                                <div class="product-price">{{ 'Rp' . number_format($item->price, 0, ',', '.') }}</div>
                                <!-- Ubah button untuk mengarah ke halaman show -->
                                <a href="{{ route('book.show', $item->id) }}" class="add-to-cart-btn">
