...
{{--             
    ...
    <div class="events-grid" id="eventsContainer">
        ...
        <div class="event-card" data-status="{{ $webinar->status }}" data-type="{{ $webinar->type }}" 
             data-date="{{ $webinar->tanggal->format('Y-m-d') }}" 
             data-price="{{ $webinar->harga }}">
            <div class="event-image">
                ...
                <span class="event-badge status-{{ $webinar->status }}">
            ...
            <div class="event-content">
                <span class="event-type type-{{ $webinar->type }}">
                    @if($webinar->type == 'seminar')
                    Seminar
                    @elseif($webinar->type == 'workshop')
                    Workshop
                ...
                <h3 class="event-title">{{ $webinar->judul }}</h3>
                <p class="event-description">{{ Str::limit($webinar->deskripsi, 120) }}</p>
                
                <div class="event-meta">
                    <div class="event-date">
                        <span class="event-day">{{ $webinar->day }}</span>
                        <span class="event-month">{{ $webinar->month }}</span>
                    </div>
                    <div class="event-details">
                        <div class="event-time">
                            <i class="fas fa-clock"></i>
                            @if($webinar->waktu_mulai && $webinar->waktu_selesai)
                            {{ \Carbon\Carbon::parse($webinar->waktu_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($webinar->waktu_selesai)->format('H:i') }} WIB
                            @else
                            {{ $webinar->tanggal->format('H:i') }} WIB
                            @endif
                        </div>
                        <div class="event-location">
                            <i class="fas @if($webinar->type == 'webinar') fa-globe @else fa-map-marker-alt @endif"></i>
                            @if($webinar->lokasi)
                            {{ $webinar->lokasi }}
                            @else
                            {{ $webinar->type == 'webinar' ? 'Online' : 'Lokasi akan diinformasikan' }}
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="event-footer">
                    ...
                    @else
                    <div class="event-price">Rp {{ number_format($webinar->harga, 0, ',', '.') }}</div>
                    @endif
