...
<section class="catalog-hero">
    <div class="container">
        <div class="hero-content">
            <h1 class="hero-title">In House Training</h1>
...
<div class="container">
    ...
    <div class="events-grid" id="eventsContainer">
        ...
        <div class="event-card" data-status="{{ $training->status }}" data-type="{{ $training->type }}" 
             data-date="{{ $training->tanggal->format('Y-m-d') }}" 
             data-price="{{ $training->harga }}">
            <div class="event-image">
                ...
                <span class="event-badge status-{{ $training->status }}">
            ...
            <div class="event-content">
                <span class="event-type type-{{ $training->type }}">
                    @if($training->type == 'seminar')
                    Seminar
                    @elseif($training->type == 'workshop')
                    Workshop
                ...
                <h3 class="event-title">{{ $training->judul }}</h3>
                <p class="event-description">{{ Str::limit($training->deskripsi, 120) }}</p>
                
                <div class="event-meta">
                    <div class="event-date">
                        <span class="event-day">{{ $training->day }}</span>
                        <span class="event-month">{{ $training->month }}</span>
                    </div>
                    <div class="event-details">
                        <div class="event-time">
                            <i class="fas fa-clock"></i>
                            {{ $training->detail->waktu_mulai ?? '09:00' }} - {{ $training->detail->waktu_selesai ?? '17:00' }} WIB
                        </div>
                        <div class="event-location">
                            <i class="fas @if($training->type == 'webinar') fa-globe @else fa-map-marker-alt @endif"></i>
                            {{ $training->lokasi }}
                        </div>
                    </div>
                </div>
                ...
                <div class="event-footer">
                    ...
                    @else
                    <div class="event-price">Rp {{ number_format($training->harga, 0, ',', '.') }}</div>
                    @endif
                    
                    @if($training->status == 'upcoming')
                    <a href="{{ route('catalog.show', $training->id) }}" class="event-action">Daftar</a>
                    @elseif($training->status == 'ongoing')
                    <a href="{{ route('catalog.show', $training->id) }}" class="event-action" style="background: var(--warning);">Join</a>
                    @else
                    <a href="{{ route('training.show', $training->id) }}" class="event-action" style="background: var(--gray);">Materi</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="pagination">
        {{ $trainings->appends(request()->query())->links('vendor.pagination.bootstrap-4') }}
    </div>
    @endif
</div>
