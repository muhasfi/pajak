...
@section('title', 'Edit Layanan Audit')
...
<div class="page-heading">
    <div class="page-title mb-4">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Edit Layanan Audit</h3>
                <p class="text-subtitle text-muted">Perbarui data layanan audit yang sudah ada.</p>
            </div>
        </div>
    </div>
    ...
    <section class="section">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('admin.audit.update', $audit->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-3">
                        <label class="form-label"><strong>Judul Layanan Audit</strong></label>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="deskripsi" rows="4" required>{{ old('deskripsi', $audit->detail->deskripsi ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <select name="file_type" class="form-select" style="max-width: 150px;" onchange="toggleFileInput(this)">
                                <option value="upload"
                                    {{ old('file_type', isset($audit->detail) && filter_var($audit->detail->file_path, FILTER_VALIDATE_URL) ? 'link' : 'upload') == 'upload' ? 'selected' : '' }}>
                                    Upload
                                </option>
                                <option value="link"
                                    {{ old('file_type', isset($audit->detail) && filter_var($audit->detail->file_path, FILTER_VALIDATE_URL) ? 'link' : 'upload') == 'link' ? 'selected' : '' }}>
                                    Link
                                </option>
                            </select>
                            ...
                            {{-- Input upload file --}}
                            @php
                                $isLink = old('file_type', isset($audit->detail) && filter_var($audit->detail->file_path, FILTER_VALIDATE_URL) ? 'link' : 'upload') === 'link';
                            @endphp
                            ...
                            <input type="text"
                                name="file_link"
                                value="{{ old('file_link', $isLink ? $audit->detail->file_path ?? '' : '') }}"
                                class="form-control {{ $isLink ? '' : 'd-none' }}"
                                placeholder="https://drive.google.com/...">

                        </div>

                        @if(isset($audit->detail->file_path))
                            <small class="text-muted">
                                File saat ini:
                                @if(filter_var($audit->detail->file_path, FILTER_VALIDATE_URL))
                                    <a href="{{ $audit->detail->file_path }}" target="_blank">Lihat Link</a>
                                @else
                                    <a href="{{ asset('storage/' . $audit->detail->file_path) }}" target="_blank">Lihat File</a>
                                @endif
                            </small>
                        @endif
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="benefits-container">
                            @if($audit->detail && $audit->detail->benefit)
