...
@section('title', 'Edit Layanan Transfer')
...
<div class="page-heading">
    <div class="page-title mb-4">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Edit Layanan Transfer</h3>
                <p class="text-subtitle text-muted">Perbarui data layanan transfer yang sudah ada.</p>
            </div>
        </div>
    </div>
    ...
    <section class="section">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('admin.transfer.update', $transfer->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="judul" class="form-control" 
                            value="{{ old('judul', $transfer->judul) }}" 
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="harga" class="form-control" 
                            value="{{ old('harga', $transfer->harga) }}" 
                            placeholder="Masukkan harga" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label"><strong>Deskripsi</strong></label>
                        <textarea class="form-control" name="deskripsi" rows="4" required>{{ old('deskripsi', $transfer->detail->deskripsi ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <select name="file_type" class="form-select" style="max-width: 150px;" onchange="toggleFileInput(this)">
                                <option value="upload"
                                    {{ old('file_type', isset($transfer->detail) && filter_var($transfer->detail->file_path, FILTER_VALIDATE_URL) ? 'link' : 'upload') == 'upload' ? 'selected' : '' }}>
                                    Upload
                                </option>
                                <option value="link"
                                    {{ old('file_type', isset($transfer->detail) && filter_var($transfer->detail->file_path, FILTER_VALIDATE_URL) ? 'link' : 'upload') == 'link' ? 'selected' : '' }}>
                                    Link
                                </option>
                            </select>
                            ...
                            {{-- Input upload file --}}
                            @php
                                $isLink = old('file_type', isset($transfer->detail) && filter_var($transfer->detail->file_path, FILTER_VALIDATE_URL) ? 'link' : 'upload') === 'link';
                            @endphp
                            ...
                            <input type="text"
                                name="file_link"
                                value="{{ old('file_link', $isLink ? $transfer->detail->file_path ?? '' : '') }}"
                                class="form-control {{ $isLink ? '' : 'd-none' }}"
                                placeholder="https://drive.google.com/...">

                        </div>

                        @if(isset($transfer->detail->file_path))
                            <small class="text-muted">
                                File saat ini:
                                @if(filter_var($transfer->detail->file_path, FILTER_VALIDATE_URL))
                                    <a href="{{ $transfer->detail->file_path }}" target="_blank">Lihat Link</a>
                                @else
                                    <a href="{{ asset('storage/' . $transfer->detail->file_path) }}" target="_blank">Lihat File</a>
                                @endif
                            </small>
                        @endif
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="benefits-container">
                            @if($transfer->detail && $transfer->detail->benefit)
                    ...
                    <div class="text-end mt-4 d-flex justify-content-end gap-2">
                        <a href="{{ route('admin.transfer.index') }}" class="btn btn-outline-primary">
